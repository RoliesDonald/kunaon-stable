...
class Room extends CI_Controller {
  ...
  // See LICENSE.TXT file for more information.
	...
	public function index(){
        ...
        $ColumnSort = array('id','id','room_name','id','id','id','id');
        ...
        if($data){
            ...
            $action = crud_action('backoffice/reference/rooms/',$row->id_main,true);
            if(table_status($row->id_main,'reserved')>0){
              $action = crud_action('backoffice/reference/rooms/',$row->id_main,false);
            }
        		...
        		$aaData[] = array(
                   ...
                   $i,
                   $row->room_name,
                   table_status($row->id_main,'total'),
                   table_status($row->id_main,'reserved'),
                   table_status($row->id_main,'empty'),
                   $action,
                 );
        	}
        }
